...
class OMLBase:
	...
	elif not (self._is_valid_schema_str(schema_str.strip()) or self._is_valid_sem_schema_str(schema_str.strip())) :
	...
	if self._is_valid_sem_schema_str(schema_str) :
		p = re.compile("([A-Za-z_][A-Za-z_0-9]*):([A-Za-z_][A-Za-z_0-9]*):")
	else :
		p = re.compile("([A-Za-z_][A-Za-z_0-9]*):([A-Za-z_][A-Za-z_0-9]*)")
	schema = p.findall(schema_str)
        names = set()
        types = []
        for s in schema:
            name, type = s
    ...
    def _is_valid_scalar_type(t):
        return t in ("bool", "int32", "uint32", "int64", "uint64", "double", "string", "blob", "guid", "datetime")
	...
	#sys.stderr.write(re.match(p, schema_str).group(0))
        return re.match(p, schema_str) is not None

    # Tests that semantic schema is valid
    #
    @staticmethod
    def _is_valid_sem_schema_str(schema_str):
	p = "[A-Za-z_][A-Za-z_0-9]*:[A-Za-z_][A-Za-z_0-9]*:(\{[A-Za-z_\-%]*:[A-Za-z_\-%]*\|[A-Za-z_\-%]*:[A-Za-z_\-%]*\|[A-Za-z_\-%]*:[A-Za-z_\-%]*\})*( +[A-Za-z_][A-Za-z_0-9]*:[A-Za-z_][A-Za-z_0-9]*:(\{[A-Za-z_\-%]*:[A-Za-z_\-%]*\|[A-Za-z_\-%]*:[A-Za-z_\-%]*\|[A-Za-z_\-%]*:[A-Za-z_\-%]*\})*)*"
        return re.match(p, schema_str) is not None
